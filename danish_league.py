...
if all_teams_selected == 'Include all available teams':
    ...
    with row13_4:
        show_me_city = st.selectbox("", ['for all Stadiums']+list(city_dict.values()), key = 'city')
    row14_spacer1, row14_1, row14_spacer2 = st.columns((.2, 7.1, .2))
    with row14_1:
        if show_me_city == 'for all Stadiums':
...
row5_spacer1, row5_1, row5_spacer2, row5_2, row5_3, row5_4, row5_spacer3  = st.columns((.2, 1.8, 0.1, 0.3, 0.7, 4.3, .2))
with row5_1:
...
with row11_1:
    st.markdown('Examine the relationship between various team attributes. For example, do teams that attempt more shots than their opponents also earn more corners? Do teams with higher possession rates tend to score more goals?')    
